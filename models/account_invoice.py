...
class AccountInvoiceLine(models.Model):
    ...
    asset_category_id = fields.Many2one('account.asset.category', string='Categoría de activos')
    asset_start_date = fields.Date(string='Fecha de inicio del activo', compute='_get_asset_date', readonly=True, store=True)
    asset_end_date = fields.Date(string='Fecha de finalización del activo', compute='_get_asset_date', readonly=True, store=True)
    asset_mrr = fields.Float(string='Ingresos mensuales recurrentes', compute='_get_asset_date', readonly=True,
        ...
        for rec in self:
            ...
            if cat:
                if cat.method_number == 0 or cat.method_period == 0:
                    raise UserError(_('El número de amortizaciones o la duración del período de '
                                      'su categoría de activos no puede ser 0.'))
